...
export class AuthManager extends APIBase {
  ...
  async signInWithCode(
    ...
    const { data, errors } = await this.fetcher.post(
      `/agnost/auth/signin-code?code=${code ?? ""}&phone=${encodeURIComponent(
        phone,
      )}`,
    );
  ...
  signInWithProvider(
    ...
    if (globalThis.window)
      globalThis.window.location.href = `${this.fetcher.getBaseUrl()}/agnost/oauth/${provider}?redirect=${encodeURIComponent(
        redirectURL,
  ...
  async signOut(sessionToken?: string): Promise<{ errors: APIError | null }> {
    try {
      const { errors } = await this.fetcher.post("/agnost/auth/signout", {
        token: sessionToken,
      });
  ...
  async signOutAll(): Promise<{ errors: APIError | null }> {
    const { errors } = await this.fetcher.post("/agnost/auth/signout-all");
  ...
  async signOutAllExceptCurrent(): Promise<{ errors: APIError | null }> {
    const { errors } = await this.fetcher.post(
      "/agnost/auth/signout-all-except",
    );
  ...
  async getAllSessions(): Promise<{
    ...
    const { data, errors } = await this.fetcher.get("/agnost/auth/sessions");
  ...
  async getUserFromDB(): Promise<{
    ...
    const { data, errors } = await this.fetcher.get("/agnost/auth/user");
  ...
  async changePassword(
    ...
    const { errors } = await this.fetcher.post("/agnost/auth/change-pwd", {
      newPassword,
      oldPassword,
    });
    return { errors };
  ...
  async getAuthGrant(accessToken?: string): Promise<{
    ...
    const { data, errors } = await this.fetcher.get(
      `/agnost/auth/grant?key=${tokenStr ?? ""}`,
    );
  ...
  async resendVerificationEmail(
    ...
    const { errors } = await this.fetcher.post("/agnost/auth/resend", {
      email,
      redirectURL,
      baseURL: this.fetcher.getBaseUrl(),
    });
  ...
  async resendVerificationCode(
    ...
    const { errors } = await this.fetcher.post(
      `/agnost/auth/resend-code?phone=${encodeURIComponent(phone)}`,
    );
    return { errors };
  }
  ...
  async sendMagicLinkEmail(
    ...
    const { errors } = await this.fetcher.post(`/agnost/auth/send-magic`, {
      email,
      redirectURL,
      baseURL: this.fetcher.getBaseUrl(),
    });
  ...
  async sendResetPwdEmail(
    ...
    const { errors } = await this.fetcher.post(`/agnost/auth/send-reset`, {
      email,
      redirectURL,
      baseURL: this.fetcher.getBaseUrl(),
    });
  ...
  async sendResetPwdCode(phone: string): Promise<{ errors: APIError | null }> {
    const { errors } = await this.fetcher.post(
      `/agnost/auth/send-reset-code?phone=${encodeURIComponent(phone)}`,
    );
    return { errors };
  }
  ...
  async sendSignInCode(phone: string): Promise<{ errors: APIError | null }> {
    const { errors } = await this.fetcher.post(
      `/agnost/auth/send-code?phone=${encodeURIComponent(phone)}`,
    );
    return { errors };
  }
  ...
  async resetPwdWithToken(
    ...
    const { errors } = await this.fetcher.post(
      `/agnost/auth/reset-pwd?key=${accessToken ?? ""}`,
      {
        newPassword,
      },
    );
  ...
  async resetPwdWithCode(
    ...
    const { errors } = await this.fetcher.post(
      `/agnost/auth/reset-pwd-code?phone=${encodeURIComponent(phone)}&code=${
        code ?? ""
      }`,
      {
        newPassword,
  ...
  async verifyPhone(
    ...
    const { data, errors } = await this.fetcher.post(
      `/agnost/auth/verify-phone?code=${code ?? ""}&phone=${encodeURIComponent(
        phone,
      )}`,
    );
