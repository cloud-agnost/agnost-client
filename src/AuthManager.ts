 ...
 *
 * You are free to design the way to authenticate your users and manage sessions in Agnost through defining your custom services. However, by default Agnost provides three methods to manage user accounts through the client library.
 *
 * 1. **Email and password based account management:** This is the default authentication method and it requires email address validation. If you set email validation as required, you need to configure the SMTP server to send confirmation emails. You can customize to enable/disable email confirmations, use your own SMTP server to send email and define your email templates.
 * 2. **Phone number and password based account management:** You can also allow your uses to sign up using their phone numbers and validate these phone numbers by sending a validation code through SMS. In order to use this method of authentication, you need to configure the SMS provider. Agnost currently supports Twilio, MessageBird, and Vonage for sending SMS messages.
 ...
 *
 * To use any of the above authentication methods you need to configure your authentication settings of your app version. You can customize these settings in Agnost Studio under **Version Settings/Authentication**.
 *
 * @export
 * @class AuthManager
 */
export class AuthManager extends APIBase {
	...
	/**
	 * Reference to the Agnost client library realtime manager
	 * @type {string}
	 */
	#client: AgnostClient;
	 ...
	 * @param {ClientOptions} options Agnost client options
	 */
	constructor(
		client: AgnostClient,
		fetcher: Fetcher,
	...
	/**
	 * Invalidates the current user session, removes local session data, and clears **Session** token request header in {@link Fetcher}. If **signInRedirect** is specified in {@link ClientOptions} when creating the Agnost api client and if the client is running in a browser, redirects the user to the sign in page.
	 */
	 ...
	 *
	 * After user clicks on the link in confirmation email, Agnost verifies the verification token sent in the email and if the email is verified successfully redirects the user to the redirect URL with an `access_token`. You can use this `access_token` token to get authentication grants, namely the user data and a new session object by calling the {@link getAuthGrant} method.
	 *
	 ...
	 * @param {string} redirectURL This parameter is required if email confirmation is enabled. After email address of the user is confirmed, the user is redirected to this URL with an `access_token` in query string parameter. The `redirectURL` needs to match with one of the configured redirect URLs in your **Version Settings/Authentication** configuration in Agnost Studio.
	 * @param {User} userData Additional user data associated with the user that is being created in the database. You can pass additional user fields with values (except email and password)  to be created in the database.
	 */
	async signUpWithEmail(
		...
		const { data, errors } = await this.fetcher.post("/auth/signup-email", {
			email,
			password,
			redirectURL,
			baseURL: this.fetcher.getBaseUrl(),
			userData,
		});

		if (errors) return { user: null, session: null, errors };

		// In case the email confirmation is disabled in app settings then Agnost returns the user and session data,
	 ...
	 * @param {string} password Password of the user, should be at least 6 characters long
	 * @param {User} userData Additional user data associated with the user that is being created in the database. You can pass additional user fields with values (except phone and password)  to be created in the database.
	 */
	async signUpWithPhone(
		...
		const { data, errors } = await this.fetcher.post("/auth/signup-phone", {
			phone,
			password,
			userData,
		});
		...
		// In case the phone number confirmation is disabled in app settings then Agnost returns the user and session data,
	...
	/**
	 * Log in an existing user using email and password. In order to use email and password based log in, the authentication provider needs to be Agnost, meaning a user with email and password credentials exists in the app database.
	 *
	...
	async signInWithEmail(
		...
		const { data, errors } = await this.fetcher.post("/auth/signin-email", {
			email,
			password,
		});
	...
	/**
	 * Log in an existing user using phone number and password. In order to use phone and password based log in, the authentication provider needs to be Agnost, meaning a user with phone and password credentials exists in the app database.
	 *
	...
	async signInWithPhone(
		...
		const { data, errors } = await this.fetcher.post("/auth/signin-phone", {
			phone,
			password,
		});
	...
	/**
	 * Log in an existing user using phone number and SMS code (OTP - one time password) that is sent to the phone. In order to use phone and password based log in, the authentication provider needs to be Agnost, meaning a user with phone and password credentials exists in the app database and *sign in using authorization codes* needs to be **enabled** in your app authentication settings. Before calling this method, you need to call the {@link sendSignInCode} method to get the SMS code delivered to the phone.
	 *
	...
	async signInWithCode(
		...
		const { data, errors } = await this.fetcher.post(
			`/auth/signin-code?code=${code ?? ""}&phone=${encodeURIComponent(phone)}`
		);
	 ...
	 *
	 * If the provider sign in completes successfully, Agnost directs the user to the redirect URL with an access token that you can use to fetch the authentication grants (e.g., user and session data).
	 *
	 ...
	 * @param {string} redirectURL After successful provider sign in, the user is redirected to this URL with an `access_token` in query string parameter. The `redirectURL` needs to match with one of the configured redirect URLs in your **Version Settings/Authentication** configuration in Agnost Studio.
	 */
	signInWithProvider(
		provider:
			| "google"
		...
		if (globalThis.window)
			globalThis.window.location.href = `${this.fetcher.getBaseUrl()}/oauth/${provider}?redirect=${encodeURIComponent(
				redirectURL
			)}&base=${encodeURIComponent(this.fetcher.getBaseUrl())}`;
	}

	/**
	 * If an input session token is <u>not</u> provided, signs out the user from the current session, clears user and session data in local storage and removes the **Session** header in {@link Fetcher}. Otherwise, signs out the user from the session identified by the input token.
	 *
	 * > *An active user session is required (e.g., user needs to be logged in) to call this method.*
	 * @param {string} [sessionToken] Session token which uniquely identifies a user session
	 */
	async signOut(sessionToken?: string): Promise<{ errors: APIError | null }> {
		try {
			const { errors } = await this.fetcher.post("/auth/signout", {
				token: sessionToken,
			});
	...
	async signOutAll(): Promise<{ errors: APIError | null }> {
		const { errors } = await this.fetcher.post("/auth/signout-all");
	...
	async signOutAllExceptCurrent(): Promise<{ errors: APIError | null }> {
		const { errors } = await this.fetcher.post("/auth/signout-all-except");
	...
	async getAllSessions(): Promise<{
		...
		const { data, errors } = await this.fetcher.get("/auth/sessions");
	...
	async getUserFromDB(): Promise<{
		...
		const { data, errors } = await this.fetcher.get("/auth/user");
	...
	async changePassword(
		...
		const { errors } = await this.fetcher.post("/auth/change-pwd", {
			newPassword,
			oldPassword,
		});
		return { errors };
	 ...
	 * @param {string} redirectURL After email address of the user is confirmed, the user is redirected to this URL with an `access_token` in query string parameter. The `redirectURL` needs to match with one of the configured redirect URLs in your **Version Settings/Authentication** configuration in Agnost Studio.
	 */
	async resendVerificationEmail(
		email: string,
		redirectURL: string
	): Promise<{ errors: APIError | null }> {
		const { errors } = await this.fetcher.post("/auth/resend", {
			email,
			redirectURL,
			baseURL: this.fetcher.getBaseUrl(),
		});
	...
	async resendVerificationCode(
		...
		const { errors } = await this.fetcher.post(
			`/auth/resend-code?phone=${encodeURIComponent(phone)}`
		);
		return { errors };
	}
	 ...
	 *
	 * When the user clicks on the link in email, Agnost verifies the validity of the magic link and if successful redirects the user to the specified redirect URL with an access token in a query string parameter named 'access_token.' You can call {@link getAuthGrant} method with this access token to create a new session object.
	 *
	 ...
	 * @param {string} email The email address of the user to send the verification email
	 * @param {string} redirectURL After successful sign in through magic link, the user is redirected to this URL with an `access_token` in query string parameter. The `redirectURL` needs to match with one of the configured redirect URLs in your **Version Settings/Authentication** configuration in Agnost Studio.
	 */
	async sendMagicLinkEmail(
		email: string,
		redirectURL: string
	): Promise<{ errors: APIError | null }> {
		const { errors } = await this.fetcher.post(`/auth/send-magic`, {
			email,
			redirectURL,
			baseURL: this.fetcher.getBaseUrl(),
		});
	 ...
	 * This method works only if email confirmation is **enabled** in your app authentication settings and the user's email address has already been verified.
	 *
	 * When the user clicks on the link in email, Agnost verifies the validity of the reset-password link and if successful redirects the user to the specified redirect URL with an access token in a query string parameter named 'access_token.' At this state your app needs to detect `action=reset-pwd` in the redirect URL and display a password reset form to the user. After getting the new password from the user, you can call {@link resetPwdWithToken} method with the access token and new password to change the password of the user.
	 *
	 * If email confirmation is **disabled** in your app authentication settings or if the user's email has not been verified, it returns an error.
	 * @param {string} email The email address of the user to send the verification email
	 * @param {string} redirectURL After successful verification of the reset-password link, the user is redirected to this URL with an `access_token` and `action` in query string parameter. The `redirectURL` needs to match with one of the configured redirect URLs in your **Version Settings/Authentication** configuration in Agnost Studio.
	 */
	async sendResetPwdEmail(
		email: string,
		redirectURL: string
	): Promise<{ errors: APIError | null }> {
		const { errors } = await this.fetcher.post(`/auth/send-reset`, {
			email,
			redirectURL,
			baseURL: this.fetcher.getBaseUrl(),
		});
	...
	async sendResetPwdCode(phone: string): Promise<{ errors: APIError | null }> {
		const { errors } = await this.fetcher.post(
			`/auth/send-reset-code?phone=${encodeURIComponent(phone)}`
		);
		return { errors };
	}
	 ...
	 *
	 * After getting the SMS code you can call the {@link signInWithCode} method. Agnost verifies the validity of the code and if successful returns the auth grants (e.g., session) of the user.
	 *
	...
	async sendSignInCode(phone: string): Promise<{ errors: APIError | null }> {
		const { errors } = await this.fetcher.post(
			`/auth/send-code?phone=${encodeURIComponent(phone)}`
		);
		return { errors };
	}
	...
	async resetPwdWithToken(
		...
		const { errors } = await this.fetcher.post(
			`/auth/reset-pwd?key=${accessToken ?? ""}`,
			{
				newPassword,
			}
		);
	...
	async resetPwdWithCode(
		...
		const { errors } = await this.fetcher.post(
			`/auth/reset-pwd-code?phone=${encodeURIComponent(phone)}&code=${
				code ?? ""
			}`,
			{
				newPassword,
	 ...
	 * @param {string} redirectURL This parameter is required if email confirmation is enabled. After the new email address of the user is confirmed, the user is redirected to this URL with success or error status in query string parameter. The `redirectURL` needs to match with one of the configured redirect URLs in your **Version Settings/Authentication** configuration in Agnost Studio.
	 */
	async changeEmail(
		...
		const { data, errors } = await this.fetcher.post(`/auth/change-email`, {
			currentPassword,
			newEmail,
			redirectURL,
			baseURL: this.fetcher.getBaseUrl(),
	...
	async changePhone(
		...
		const { data, errors } = await this.fetcher.post(`/auth/change-phone`, {
			currentPassword,
			newPhone,
		});
	...
	async verifyPhone(
		...
		const { data, errors } = await this.fetcher.post(
			`/auth/verify-phone?code=${code ?? ""}&phone=${encodeURIComponent(phone)}`
		);
