 ...
 *
 * The configuration parameters of the realtime module is specified when creating the Agnost client library instance. In particular three key parameters affect how realtime messaging works in your apps.
 *
...
export class RealtimeManager extends APIBase {
  ...
  /**
   * Keeps a reference to the latest user data that is updated using the {@link updateProfile} method.
   * @type {number}
   */
  #userData: any;
  ...
  async getMembers(
    ...
    checkRequired("channel", channel, true);
    return await this.fetcher.post(`/agnost/realtime/get-members`, {
      channel,
    });
  }
