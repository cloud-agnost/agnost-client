...
/**
 * HTTP client for the browser, Node or React Native. Created by {@link AgnostClient} during initialization. The client library uses [cross-fetch](https://www.npmjs.com/package/cross-fetch) under the hood to make requests to you app's execution environment.
 *
 * Fetcher adds the provided apiKey to an **Authorization** header and sends it in all RESTful API requests to your backend app.
 *
...
export class Fetcher {
	/**
	 * Reference to the Agnost client object
	 * @protected
	 * @type {AgnostClient}
	 */
	protected apiClient: AgnostClient;
	/**
	 ...
	 */
	constructor(apiClient: AgnostClient, restUrl: string, headers: KeyValuePair) {
	...
	upload(
		...
		return new Promise((resolve, reject) => {
			if (typeof XMLHttpRequest === "undefined") {
				return reject(
					new Error(
						"XMLHttpRequest is not defined. This method can only be called from clients where `XMLHttpRequest` object is available (e.g., browsers)."
					)
				);
			}
